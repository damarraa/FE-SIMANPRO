...
const ResetPasswordPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.post("/v1/reset-password", formData);
      ...
      if (err.response && err.response.status === 422) {
        ...
        const msg = errors?.password ? errors.password[0] : (errors?.email ? errors.email[0] : "Validasi gagal.");
        setError(msg);
      } else {
      ...
      <div className="max-w-md w-full space-y-8">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-gray-600">
            Buat password baru untuk akun {formData.email}
          </p>
        </div>
